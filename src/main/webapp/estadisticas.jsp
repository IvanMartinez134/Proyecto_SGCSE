    ...
    fetch('/pruebaGit_war_exploded/obtenerTramites')
        ...
        .then(data => {
            ...
            data.forEach(item => {
                ...
                seriesData[item.tipo_de_tramite].push({ time: item.fecha, value: item.cantidad });
            });
            ...
            Object.keys(seriesData).forEach(tipo => {
                const areaSeries = chart.addAreaSeries({
                    topColor: colors[tipo].topColor,
                    bottomColor: colors[tipo].bottomColor,
                    lineColor: colors[tipo].lineColor,
                    lineWidth: 2 // Ajusta el grosor de la línea
                });
                areaSeries.setData(seriesData[tipo]);
            });

            // Configura el formateador del eje X
